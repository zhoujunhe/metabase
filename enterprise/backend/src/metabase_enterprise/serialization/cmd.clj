...
(defn load
  ...
  (prn "load" legacy v2 args)
  (when (and v2 legacy)
    (log/error (trs "ERROR load cannot be both --legacy and --v2"))
    (throw (ex-info "load cannot be both --legacy and --v2" {})))
  (if legacy
    (v1-load path args)
    (v2-load path args)))
...
(defn dump
  ...
  (when (and v2 legacy)
    (log/error (trs "ERROR dump cannot be both --legacy and --v2"))
    (throw (ex-info "dump cannot be both --legacy and --v2" {})))
  (if legacy
